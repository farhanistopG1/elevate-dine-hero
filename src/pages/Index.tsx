...
const defaultContent: ClientData = {
  ...
  about: {
    ...
    paragraph1:
      "At Sage, we blend tradition with innovation to create unforgettable dishes. Our philosophy is rooted in using the freshest, locally-sourced ingredients to craft a menu that is both sophisticated and approachable.",
    paragraph2:
      "Our commitment to quality is unwavering, from the kitchen to your table. We invite you to join us for an evening of culinary excellence in an atmosphere of relaxed elegance.",
  },
};
...
const Index = () => {
  ...
  useEffect(() => {
    ...
    if (clientHandle) {
      ...
      // ✅ Force the browser to bypass *all* caches
      fetch(dataUrl, { cache: "reload" })
        ...
        .catch((err) =>
          console.error("Could not load client data, using default.", err)
        )
  ...
  if (isLoading) {
    return (
      <div style={{ textAlign: "center", padding: "10rem", fontSize: "1.5rem" }}>
        Loading Preview...
      </div>
    );
  }
